...
public class EmailVerificationImpl implements EmailVerificationInterface {
	...
	public EmailVerificationImpl(JavaMailSender javaMailSender) {
	...
	public Mono<Void> sendVerificationEmail(User user) {
		...
		return sendEmail(user.getEmail(), "Email Verification", emailContent);
	}
	...
	private Mono<Void> saveTokenToDatabase(String email, String value, long expirationTimeMillis) {
		...
		return webClient.post().uri("http://localhost:8080/api/token").contentType(MediaType.APPLICATION_JSON)
				.bodyValue(new Token(value, email, expirationTimeMillis)).retrieve().toBodilessEntity().then()
				.doOnError(error -> {
	...
	private String buildEmailContent(String userEmail, String verificationUrl) {
		...
		return "<html><body><p>Hello,</p><p>Please click the following link to verify your email: <a href='"
				+ verificationUrl + "'>Verify Email</a></p></body></html>";
	}
